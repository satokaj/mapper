...
namespace SensorHelpers {
	...
	using RotationVector = std::array<qreal, 4>;
	/**
	 ...
	 */
	Matrix getRotationMatrix(const Vector& gravity, const Vector& geomagnetic, bool& ok)
	{
		auto Ax = gravity[0];
		auto Ay = gravity[1];
		auto Az = gravity[2];
		...
		// When normH is close to 0, the device is in atypical state or location
		// (e.g. close to magnetic north pole). Typical values are > 100.
		ok = normH >= 0.1;
		if (ok)
		{
	 ...
	 */
	Matrix getRotationMatrixFromVector(const RotationVector& rotationVector)
	{
...
public:
	...
	bool filter(QGyroscopeReading* reading) override
	{
		...
		if (last_gyro_timestamp > 0)
		{
			...
			const auto deltaRotationVector = SensorHelpers::RotationVector {
...
private:
	...
	public:
		...
		void filter()
		{
			...
			bool ok = false;
			const auto R = SensorHelpers::getRotationMatrix(acceleration, geomagnetic, ok);
			if (!ok)
				return;
